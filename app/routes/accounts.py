...
def create_account(payload: AccountCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    if payload.type not in ("cash", "bank", "dps", "fdr"):
...
def update_account(account_id: int, payload: AccountUpdate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    account = db.query(Account).filter(Account.id == account_id).first()
    if not account:
        raise HTTPException(status_code=404, detail="Account not found")
    if payload.type and payload.type not in ("cash", "bank", "dps", "fdr"):
        raise HTTPException(status_code=400, detail="Invalid account type")
